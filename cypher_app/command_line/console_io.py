def menu_selection(menu_options, with_quit=False):
    ...
    for index, option in enumerate(menu_options):
        if(not with_quit):
            ...
            print(index, ")", option)
    message = "Enter A Number Between " + \
...
def get_input(message, allow_empty=False, filter=None):
    is_valid_input = False
    user_input = None
    while(not is_valid_input):
        user_input = input(message + ": ").strip()
        is_valid_input = allow_empty or (
            not allow_empty and user_input not in '')
        is_valid_input = is_valid_input and (
            filter is None or filter(user_input))
    return user_input
